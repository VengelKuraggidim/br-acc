"""Tests for the gated federal /api/v1/public/* endpoints.

These tests exercise ``bracc._federal.routers.public`` (formerly
``bracc.routers.public``) which is only mounted when
``ENABLE_FEDERAL_ROUTES=true``. They were moved out of
``tests/unit/test_public_mode.py`` together with the router gate so the
default ``make test-api`` run does not try to call routes that no longer
exist.

The suite is auto-marked ``federal`` by ``api/tests/_federal/conftest.py``;
use ``make test-api-federal`` to run it.
"""
...
async def test_public_meta_endpoint(client: AsyncClient) -> None:
    with patch(
        "bracc._federal.routers.public.execute_query_single",
...
async def test_public_patterns_company_endpoint(client: AsyncClient) -> None:
    with patch("bracc._federal.routers.public.settings.patterns_enabled", False):
...
async def test_public_patterns_company_endpoint_when_enabled(client: AsyncClient) -> None:
    with (
        patch("bracc._federal.routers.public.settings.patterns_enabled", True),
        patch(
            "bracc._federal.routers.public.execute_query_single",
            new_callable=AsyncMock,
            return_value={
        ...
        patch(
            "bracc._federal.routers.public._PUBLIC_PROVIDER.run_pattern",
            new_callable=AsyncMock,
            return_value=[
                PatternResult(
...
async def test_public_graph_company_filters_person_nodes(client: AsyncClient) -> None:
    with (
        ...
        patch(
            "bracc._federal.routers.public.execute_query",
            new_callable=AsyncMock,
            return_value=[
                {
                    "nodes": [
