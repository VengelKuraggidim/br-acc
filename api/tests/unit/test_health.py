...
async def test_meta_sources(client: AsyncClient) -> None:
    ...
    assert len(data["sources"]) == 125
    source_ids = [s["id"] for s in data["sources"]]
    assert "cnpj" in source_ids
    assert "brasilapi_cnpj" in source_ids
    ...
    assert "querido_diario_go" in source_ids  # querido_diario federal deprecated 2026-04-18
...
async def test_meta_stats(client: AsyncClient) -> None:
    ...
    assert data["data_sources"] == 125
    assert data["implemented_sources"] == 65
